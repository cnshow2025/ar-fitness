...
- 第一次進關卡會做「九宮格校正」：站在中央停 1.5 秒，然後依指示往前、後、左、右各踩一步並停住，App 記錄您實際踩到的位置來建立專屬的 3×3 九宮格（斜角由前後左右合成）。方向與步幅都以您實際的動作為準，校正結果存在手機，之後同一個鏡頭與位置會自動沿用；也可按「略過，用自動估計」用腿長推算。預設以「跳舞墊」顯示在腳邊，方向像照鏡子：上排＝往後退、下排＝往手機走、左右＝您的左右，腳的位置會對應畫在墊子上；設定頁可切換成「上排＝往手機走」（跳舞機習慣）、改回貼地透視格，或調整格子大小（一步要跨多遠）。
